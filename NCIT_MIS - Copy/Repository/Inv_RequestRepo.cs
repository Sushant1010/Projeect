...
namespace NCIT_MIS.Repository
{
    public class Inv_RequestRepo
    {
        ...
        public RequestDetail GetRequestListDetail(int depid)
        {
            ...
            {
                detail.RequestList[i].RequestItemByRequestTokenIdList = GetRequestItemListByRequestTokenId(depid, detail.RequestList[i].TokenId);
            }
            return detail;
        }
        ...
        public List<RequestItemByRequestTokenId> GetRequestItemListByRequestTokenId(int depid, int tokenId)
        {
        ...
        public RequestVM GetRequestToken(int depId, int tokenid)
        {
            string sql = "select TokenId,RequestToken,ApprovedByAdmin,RequestStatus, LocationId" +
                ...
                " where 1 = 1 and IsDeleted = 0 and TokenId = " + tokenid + " and DepartmentId = " + depId;
        ...
        {
            string sql = "select " +
                " r.RequestId, ai.CategoryId, ac.CategoryName," +
                " r.TokenId, r.RequestItemId, ai.ItemName, r.RequestQuantity, rt.LocationId" +
                " from Request r" +
                " left join INV_Item ai on ai.ItemId = r.RequestItemId" +
